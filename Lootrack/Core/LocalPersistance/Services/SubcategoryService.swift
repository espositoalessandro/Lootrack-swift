...
final class SubcategoryService {
    ...
    func reconcile(
        categoryId: UUID,
        desired: [(id: UUID, name: String)]
    ) throws {
        let normalizedDesired = try validateAndNormalize(
            desired
        )

        let allSubcategories = try modelContext.fetch(
    ...
    private func delete(
         ...
         */
        try clearTransactions(
            referencing: subcategory.id,
            at: now
        )
    }

    private func clearTransactions(
        referencing subcategoryId: UUID,
        at now: Date
    ) throws {
